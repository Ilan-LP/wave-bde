...
The backend exposes `GET /health` plus `POST /auth/login`, `/auth/refresh`, `/auth/logout` (see **Authentication** below), `GET /me/qrcode` (see **Per-User QR Code** below), `GET /products`, `GET /products/all`, `POST /products`, `PATCH /products/:id`, `PATCH /products/:id/active` (see **Product Catalog** below), `POST /buvette/scan` (see **Buvette Scan + Deduction** below), the Prisma schema/migrations (see **Data model** below), RBAC middleware (`authenticate`, `requireRole`, `requirePoleAccess` — see **RBAC Middleware** below), a generic audit logging middleware (`auditLog` — see **Audit Logging** below), and a daily background job that exports `AuditLog` rows to Google Drive as a passive backup (see **Audit Log Export** below). This is the first slice of Build Order step 2 (points-only; SumUp/card payment is still a separate future task — see **Buvette Scan + Deduction**). The buvette cashier frontend (`apps/buvette`) is built — see **Buvette Cashier Frontend**. The other 3 frontends remain scaffolded shells (Vite/React/TS/Tailwind) with no real UI or routes yet.
...
2. Buvette, functional end to end including SumUp integration — no treasury dashboard needed yet, raw logs are sufficient at this stage. **In progress**: points-side scan + deduction (`POST /buvette/scan`, see **Buvette Scan + Deduction**) and the cashier frontend (`apps/buvette`, see **Buvette Cashier Frontend**) are done; SumUp/card payment is not started (the frontend's card mode is a placeholder).
...
- **Points vs. real currency for cash/card sales**: the buvette cashier frontend (see **Buvette Cashier Frontend**) displays `Product.pricePoints` uniformly as "pts" across all three payment modes, including cash (the change-counting aid) and card. This assumes a 1:1 points-to-currency peg for non-scan payments, which is never stated anywhere in the spec — points are otherwise documented as a balance deducted only via scan/QR. Resolve with the project owner: is a fixed conversion rate intended, or should cash/card totals be a separate currency amount (which would need its own field on `Product`)?
