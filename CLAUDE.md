...
- `GOOGLE_SERVICE_ACCOUNT_JSON` — the full contents of a Google Cloud service-account key file (Drive API enabled, `drive.file` scope), minified to one line. `env.ts` validates it's present *and* parses as JSON (`requireJsonEnv`); `jobs/googleDrive.ts` separately validates the parsed object actually has `client_email`/`private_key` string fields before constructing the `google-auth-library` `JWT` client, since JSON-parseable isn't the same as shaped-correctly.
...
Repo-level prep for deploying to an always-on Debian home machine (not a rented VPS) — done 2026-09-07. This is config/documentation only: nothing has been executed, connected to, or configured on any real machine yet, and no CI/CD auto-deploy exists (out of scope, not planned as part of this prep). See `DEPLOY.md` for the manual checklist of what still has to happen on the actual machine.

**Reverse proxy: an existing Caddy instance, not one this repo runs.** The target Debian machine already runs Caddy (as a Docker container, with a single Caddyfile) for another, unrelated project — discovered *after* the initial 2026-09-07 deployment prep, which had wrongly assumed a clean machine and included a standalone `caddy` service. Corrected the same day (see **Known issues / follow-ups**): this deployment now integrates with that existing instance instead of running a second reverse proxy. `docker-compose.prod.yml` (root) — production variant of `docker-compose.yml`: `postgres` and `backend` don't publish ports to the host; `backend` is reachable only via two Docker networks — the default one (to reach `postgres`) and `caddy_net`, an `external: true` network whose real name (`CADDY_EXTERNAL_NETWORK` env var, placeholder `caddy_net`) must match whatever network the existing Caddy container is already attached to, confirmed on the actual machine before deploying. Neither service has insecure default credentials baked in — every secret must come from `.env.production`.

**Files:** `deploy/wave.caddy` — not a standalone Caddyfile, an *additive site block* meant to be appended into the existing Caddy container's own Caddyfile (manual `DEPLOY.md` step), reverse-proxying `{$DOMAIN}` to `wave-backend-prod:{$BACKEND_PORT}` over the shared `caddy_net` network. `deploy/Dockerfile.caddy` (a custom `xcaddy` build adding the Cloudflare DNS module) was removed in the correction — unnecessary, since the existing Caddy binary already has that module built in (confirmed 2026-09-07 for this specific machine; re-verify if that instance is ever rebuilt/replaced).

**HTTPS/certs: Let's Encrypt via the existing Caddy's automatic HTTPS, using the DNS-01 challenge against Cloudflare** (`deploy/wave.caddy`'s `tls { dns cloudflare {$CF_API_TOKEN} }`) — **not** HTTP-01. This is a home connection without a guaranteed static IP, so cert issuance deliberately doesn't depend on Let's Encrypt reaching this machine over inbound port 80; DNS-01 only needs outbound access from the machine to Cloudflare's API. Port 443 inbound is still required to actually *serve* HTTPS traffic — already forwarded for the existing project, per the 2026-09-07 correction's findings; not a new requirement this deployment introduces. Dynamic DNS (keeping the domain's `A` record pointed at the home connection's current IP) is deliberately **not** containerized here — it's a manual `DEPLOY.md` step (router-level DDNS client or a host-side updater hitting Cloudflare's API), so this prep doesn't add a service beyond the backend/Postgres it actually owns.

**Env vars, production (`.env.production.example`, root):** `POSTGRES_USER`/`POSTGRES_PASSWORD`/`POSTGRES_DB` (no defaults, unlike dev), `BACKEND_PORT`, `JWT_SECRET`/`JWT_REFRESH_SECRET`, `GOOGLE_SERVICE_ACCOUNT_JSON`/`GOOGLE_DRIVE_FOLDER_ID` (same fail-fast requirements as dev — see **Authentication** and **Audit Log Export** above), plus three for the reverse-proxy integration: `CADDY_EXTERNAL_NETWORK` (placeholder `caddy_net` — the existing Caddy container's Docker network, see above), `DOMAIN` (placeholder `wave.example.org` — no real domain chosen yet), `CF_API_TOKEN` (Cloudflare API token scoped to `Zone:DNS:Edit` only, not the Global API Key). Note `DOMAIN`/`CF_API_TOKEN` aren't read by `docker-compose.prod.yml` itself anymore (no `caddy` service left to consume them) — they document what must land in the *existing* Caddy container's own environment, or be substituted directly into `deploy/wave.caddy`, per `DEPLOY.md`. `ACME_EMAIL` was removed — the existing Caddyfile's global options block already owns that. `.env.production` itself is git-ignored (added to `.gitignore`/`.dockerignore` alongside the existing `.env`/`.env.*.local` entries, which didn't already cover this filename).

**Unresolved before actual deployment:** no real domain is registered yet and Cloudflare isn't yet the confirmed DNS host — `DOMAIN`/`CF_API_TOKEN` are placeholders pending that decision. The exact Docker network name for `CADDY_EXTERNAL_NETWORK` is also a placeholder — must be read off the real machine. See `DEPLOY.md` for the full manual checklist (Docker install, DNS/Cloudflare setup, dynamic DNS, router port forwarding, appending the site block to the existing Caddyfile, Postgres volume backups — the daily Google Drive export only covers `AuditLog` rows, not a full DB backup).
