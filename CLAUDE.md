...
The backend exposes `GET /health` plus `POST /auth/login`, `/auth/refresh`, `/auth/logout` (see **Authentication** below), `GET /me/qrcode` (see **Per-User QR Code** below), `GET /products` (see **Product Catalog** below), `POST /buvette/scan` (see **Buvette Scan + Deduction** below), the Prisma schema/migrations (see **Data model** below), RBAC middleware (`authenticate`, `requireRole`, `requirePoleAccess` — see **RBAC Middleware** below), a generic audit logging middleware (`auditLog` — see **Audit Logging** below), and a daily background job that exports `AuditLog` rows to Google Drive as a passive backup (see **Audit Log Export** below). This is the first slice of Build Order step 2 (points-only; SumUp/card payment is still a separate future task — see **Buvette Scan + Deduction**). The buvette cashier frontend (`apps/buvette`) is built — see **Buvette Cashier Frontend**. The other 3 frontends remain scaffolded shells (Vite/React/TS/Tailwind) with no real UI or routes yet.
...
Implemented in `apps/backend/src/routes/products.ts`, mounted in `src/app.ts` via `app.use("/products", productsRouter)`. Tests in `src/__tests__/routes/products.test.ts` (Vitest + Supertest, Prisma mocked). Added alongside the buvette cashier frontend (see **Buvette Cashier Frontend**) — the `Product` model existed since migration `20260907145039_add_product`, but nothing served it until the till needed a catalog to render product buttons from.

**Endpoint:** `GET /products`, behind `authenticate` + `requireRole("RESPONSABLE_POLE", "MEMBRE_POLE")` — same "any logged-in member" RBAC shape as `POST /buvette/scan` below (BUREAU bypasses automatically). Returns `{ products: [{ id, name, pricePoints }] }`, filtered to `isActive: true` and ordered by name. No pagination — the buvette catalog is expected to stay small.
